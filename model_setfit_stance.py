...
# Load another instance of the same model (presumably for finetuning)
finetuned_model = SentenceTransformer("all-mpnet-base-v2")

# Apply a PEFT adapter to the first layer of the model
finetuned_model[0].auto_model = PeftModel.from_pretrained(
    finetuned_model[0].auto_model,  # Original base model
    "vahidthegreat/StanceAware-SBERT"  # LoRA adapter checkpoint
)
